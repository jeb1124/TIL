...
class User2{
	public void go() throws ZeroDivide{
		// 스캐너로 i,j에 해당하는 값을 받아 들인다고 가정
		int i =10; int j = 0;
		
		System.out.println("1. Scanner로 입력받는 j값이 0이면 안되는 데...");
		// j값이 0이면 예외를 고의적으로 발생시켜서 더 이상 진행되지 않도록 처리..예외객체를 생성
		if(j==0) throw new ZeroDivide("분모가 0이되면 안됩니다.");
		
		// 아래 부분 어딘가에서 i/j j가 분모가 되는 연산이 진행된다고 가정
	}
}
