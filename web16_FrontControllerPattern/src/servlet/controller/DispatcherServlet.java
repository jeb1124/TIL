...
public class DispatcherServlet extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//1. 가장 먼저 해야하는 일은 클라이언트로부터 어떤 요청이 들어오는지를 알아야 한다..command값 받아온다.
		
		//1. RegisterServlet의 doProcess(){} 안에 있는 코드를 여기에 직접 작성...권장하지 않음.(코드 가독성 떨어짐.)
		//2. method로 하나 뽑아내서 그 메소드 안에서 RegisterServlet의 doProcess(){} 안에 있는 코드를 작성..이 방법이 좋음.(가독성이 높아지고, 구조적 파악 용이성 --(유지보수성이 높음.))  
	...
	protected String login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//세션에 저장해야지만 로그인 동안 인증을 계속 유지할 수 있음.
